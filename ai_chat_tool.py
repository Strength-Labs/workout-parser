...
def run_ai_chat(token, user_id, client, exercise_map):
    ...
    console.print("[bold green]AI Chat started. Type 'exit' to quit, or 'save' to export the last response to a file.[/bold green]")
    ...
    while True:
        ...
        try:
            response = client_ai.chat.completions.create(
                ...
                max_tokens=1500   # Limit to keep responses manageable
            )
            ...
            # Offer to save if it looks like a workout plan (heuristic: checks for markup patterns)
            if "Workout Date:" in ai_response or "x" in ai_response or "@" in ai_response:
                save_choice = input("[dim]This looks like a workout plan. Save to file for upload? (y/n): [/dim]").lower()
                if save_choice == 'y':
                    with tempfile.NamedTemporaryFile(mode='w+', delete=False, suffix='.txt') as temp_file:
                        temp_file.write(ai_response)
                        temp_path = temp_file.name
                    console.print(f"[green]Saved to temporary file: {temp_path}[/green]")

                    # Open in editor
                    editor_cmd = get_default_editor()
                    subprocess.run(editor_cmd + [temp_path], check=False)

                    # Offer to parse and upload
                    upload_choice = input("[dim]Upload this workout now? (y/n): [/dim]").lower()
                    if upload_choice == 'y':
