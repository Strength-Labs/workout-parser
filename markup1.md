...
* **Ignored Comments**: These comments are for conversational purposes and are ignored by the `workout_parser.py` script during re-upload.
    ...
    * **Private Coach Notes**: Indented lines beginning with `>`. See below.
        * Example: `> He should go up 5% on squats next week.`

### Coach Strategy Notes (Invisible to Client)

**Purpose**: These notes explain coaching rationale, adjustments, and future planning. They are **invisible to clients** during API upload but provide critical context for the AI coach.

**Format**: Indented lines beginning with `>` 
...
Squat
3x5 @ 405
    > Strategy: Building base strength using Rippetoe linear progression. Client recovering from knee issue, so conservative start.
    > Next week: If RPE < 7 on all sets, increase by 5lbs. Monitor knee stability.
(1x5 @ 405)
(1x5 @ 410)  # Client pushed beyond prescription
    > Note: Client exceeded prescription - excellent sign of recovery. Maintain progression but watch for overconfidence.
...
**AI Usage Guidelines**:
- **Preserve existing coach notes** when extending workouts
- **Add new coach notes** explaining your reasoning for changes
- **Reference client performance** in accomplished sets when making adjustments
- **Maintain progression logic** based on the selected methodology
- **Flag potential issues** (form, recovery, plateaus) for human coach review
