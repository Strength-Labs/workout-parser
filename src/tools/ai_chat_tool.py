...
from src.api_client import get_workout_history_headless
...
from src.settings import get_default_editor, get_llm_credentials
from src.directory_migration import get_client_dir, get_coaching_context_dir
from src.encoding_utils import safe_open, safe_temp_file, read_text_file, write_text_file
...
def create_chat_session_log(client_id: int, client_name: str) -> tuple:
    """Create a new chat session log file with timestamp.
    ...
    Returns:
        (log_file_path, session_start_time) tuple
    """
    # Create ai_chats directory in client folder
    client_dir = get_client_dir(client_id)
    ai_chats_dir = os.path.join(client_dir, "ai_chats")
    os.makedirs(ai_chats_dir, exist_ok=True)

    # Generate timestamp for filename (e.g., "2025-10-19_2-30pm.md")
    now = datetime.now()
    timestamp = now.strftime("%Y-%m-%d_%I-%M%p").lower()
    log_filename = f"{timestamp}.md"
    log_path = os.path.join(ai_chats_dir, log_filename)

    # Handle rare case of duplicate filename (if two sessions start in same minute)
    counter = 1
    while os.path.exists(log_path):
        log_filename = f"{timestamp}-{counter}.md"
        log_path = os.path.join(ai_chats_dir, log_filename)
        counter += 1

    # Create initial markdown file with header
    header = f"""# AI Chat Session - {client_name}
...
def log_chat_exchange(log_path: str, user_prompt: str, ai_response: str):
    """Append a chat exchange to the session log.
...
"""
    ...
    # Append to file (more efficient than read+write for long sessions)
    with safe_open(log_path, 'a') as f:
        f.write(exchange)

def get_first_user_prompt(file_path: str) -> str:
    """Extract the first user prompt from a chat log for preview.

    Args:
        file_path: Path to the chat log file

    Returns:
        First line of the first user prompt (truncated to 80 chars)
    """
    try:
        content = read_text_file(file_path)
        # Find first "### [time] User:" marker
        import re
        ...
        if match:
            ...
            if len(first_prompt) > 80:
                return first_prompt[:77] + "..."
            return first_prompt
        return "(empty session)"
    except Exception:
        return "(error reading file)"

def search_chat_history(client_id: int, search_term: str) -> list:
    """Search all chat history files for a term.
    ...
    Returns:
        List of tuples: (filename, match_snippet, full_path)
    """
    import re
    ...
    if not os.path.exists(ai_chats_dir):
        return []

    results = []
    files = sorted([f for f in os.listdir(ai_chats_dir) if f.endswith('.md')], reverse=True)

    for filename in files:
        file_path = os.path.join(ai_chats_dir, filename)
        try:
            content = read_text_file(file_path)
            # Case-insensitive search
            if search_term.lower() in content.lower():
                # Find the first match and extract context (50 chars before and after)
                match_pos = content.lower().find(search_term.lower())
                start = max(0, match_pos - 50)
                end = min(len(content), match_pos + len(search_term) + 50)
                snippet = content[start:end]

                # Clean up snippet (remove newlines, truncate)
                snippet = ' '.join(snippet.split())

                # Limit snippet length to avoid display issues
                ...
                if start > 0:
                    snippet = "..." + snippet
                elif end < len(content) and not snippet.endswith("..."):
                    snippet = snippet + "..."

                # Highlight the match term in the snippet (only first occurrence to avoid clutter)
                try:
                    snippet = re.sub(
                        f"({re.escape(search_term)})",
                        r"[\1]",
                        snippet,
                        count=1,  # Only highlight first match
                        flags=re.IGNORECASE
                    )
                except re.error:
                    # If regex fails for any reason, continue without highlighting
                    pass

                results.append((filename, snippet, file_path))
        except Exception:
            continue

    return results

def browse_chat_history(client):
    """Browse and search AI chat history for a client.

    Args:
        client: Client dictionary with id and full_name
    """
    client_dir = get_client_dir(client['id'])
    ai_chats_dir = os.path.join(client_dir, "ai_chats")

    if not os.path.exists(ai_chats_dir):
        console.print(f"[yellow]No chat history found for {client['full_name']}[/yellow]")
        input("Press Enter to continue.")
        return

    while True:
        console.print(f"\n[bold]AI Chat History - {client['full_name']}[/bold]")
        console.print("  [bold]1.[/bold] Browse recent sessions")
        console.print("  [bold]2.[/bold] Search chat history")
        console.print("  [bold]3.[/bold] Back to main menu")
        ...
        if choice == '1':
            ...
            console.print(f"[dim]Found {len(files)} session(s)[/dim]\n")

            # Display first 20 sessions with first prompt excerpt
            display_files = files[:20]
            for i, filename in enumerate(display_files, 1):
                file_path = os.path.join(ai_chats_dir, filename)
                first_prompt = get_first_user_prompt(file_path)

                # Parse timestamp from filename (e.g., "2025-10-19_2-30pm.md")
                display_name = filename.replace('.md', '').replace('_', ' ').replace('-', ':')

                console.print(f"[bold]{i:2d}.[/bold] {display_name}")
                console.print(f"     [dim]{first_prompt}[/dim]")

            if len(files) > 20:
                console.print(f"\n[dim]... and {len(files) - 20} more session(s)[/dim]")

            # Let user select a session to view
            selection = input("\nEnter session number to view (or Enter to go back): ").strip()
            if selection:
                try:
                    idx = int(selection) - 1
                    if 0 <= idx < len(display_files):
                        file_path = os.path.join(ai_chats_dir, display_files[idx])
                        content = read_text_file(file_path)
                        ...
                        action = input("Press Enter to continue, or 'e' to edit in editor... ").strip().lower()
                        if action == 'e':
                            # Open in editor
                            editor_cmd = get_default_editor()
                            try:
                                subprocess.run(editor_cmd + [file_path], check=False)
                                console.print("[green]Closed editor.[/green]")
            ...
            results = search_chat_history(client['id'], search_term)
            ...
            console.print(f"\n[bold]Found {len(results)} matching session(s):[/bold]\n")

            for i, (filename, snippet, file_path) in enumerate(results, 1):
                display_name = filename.replace('.md', '').replace('_', ' ').replace('-', ':')
                console.print(f"[bold]{i:2d}.[/bold] {display_name}")
                console.print(f"     [dim]{snippet}[/dim]\n")

            # Let user select a result to view
            selection = input("Enter session number to view (or Enter to go back): ").strip()
            if selection:
                try:
                    idx = int(selection) - 1
                    if 0 <= idx < len(results):
                        _, _, file_path = results[idx]
                        content = read_text_file(file_path)
...
def run_ai_chat(token, user_id, client, exercise_map):
    ...
    log_path, session_start = create_chat_session_log(client['id'], client['full_name'])
    console.print(f"[dim]Session log: {os.path.basename(log_path)}[/dim]")
    ...
    while True:
        ...
        try:
            ...
            # Log this exchange to the session file
            log_chat_exchange(log_path, user_input, ai_response)
