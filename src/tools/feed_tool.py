...
from src.settings import get_default_editor # added to make editor defaults more sensible

# Import shared functions from the api_client.py file
from src.api_client import API_BASE_URL, clean_text, clear_screen
...
def fetch_and_aggregate_data(token, client, feed_data_lock, feed_data):
    try:
        ...
        console.print(f"[red]Error in fetch_and_aggregate_data: {e}[/red]")
        with feed_data_lock:
            feed_data["is_refreshing"] = False
...
def run_feed(token, coach_user_id, client):
    ...
    offset = max(0, len(feed_data["events"]) - page_size)
    refresh_thread = None
    ...
    while True:
        ...
        if is_refreshing:
            ...
            with feed_data_lock:
                display_data = copy.deepcopy(feed_data)
            is_refreshing = refresh_thread is not None and refresh_thread.is_alive()
        if not is_refreshing and display_data["is_refreshing"]:
            offset = max(0, len(display_data["events"]) - page_size)
            with feed_data_lock:
                feed_data["is_refreshing"] = False
            display_data["is_refreshing"] = False
        display_feed(
            display_data["events"],
            coach_user_id,
            search_query,
        ...
        if is_refreshing:
            console.print("\n[bold]Feed Refreshing...[/bold]")
            console.print("[dim]Available: [bold]q[/bold] - Back to tool menu; [bold]u[/bold] - Force refresh[/dim]")
            prompt_text = "\n[dim]Refreshing... (q to go back)[/dim]\n> "
        else:
            console.print("\n[bold]Commands (press Enter after):[/bold]")
            console.print("[bold]m <text>[/bold] - Send message")
            console.print("[bold]c <ID> <text>[/bold] - Reply to comment")
            console.print("[bold]/<query>[/bold] - Search; n/N next/prev; s clear")
            console.print("[bold]j/k[/bold] - down/up; [bold]pgdn/pgup[/bold] - page; [bold]gg[/bold]/[bold]end[/bold] - top/bottom")
            console.print("[bold]v[/bold] - Single-keystroke navigation mode (j/k/space/b/g/G, q to exit)")
            console.print("[bold]x [filename][/bold] - Export feed to text file; [bold]o [filename][/bold] - Open exported file in $EDITOR")
            console.print("[bold]u[/bold] - Force refresh")
            console.print("[bold]q[/bold] - Back to tool menu")
            prompt_text = "\n> "

        command_line = console.input(prompt_text)
        ...
        if is_refreshing and command != 'u':
            continue
        ...
        if command == 'j':
            ...
            search_query = command[1:].strip() if len(parts) > 1 else ""
            rebuild_matches(display_data["events"], search_query)
            nav_mode(display_data["events"])
            offset = max(0, len(display_data["events"]) - page_size)
        elif command in ('n', 'N'):
            ...
            pass
        if should_refresh and (refresh_thread is None or not refresh_thread.is_alive()):
            with feed_data_lock:
