...
if [ ! -f "assets/app-icon.icns" ]; then
    echo "❌ Custom icon not found (assets/app-icon.icns)"
    echo "   Please make sure assets/app-icon.icns is in the project directory"
    exit 1
fi
...
cp "assets/app-icon.icns" "$RESOURCES_DIR/"
...
- Custom icon: Yes (assets/app-icon.icns)
...
- Contents/Resources/assets/app-icon.icns (custom icon)
