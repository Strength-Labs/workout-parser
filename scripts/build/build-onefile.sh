...
if [ -f "assets/app-icon.icns" ]; then
    echo "🎨 Found custom icon: assets/app-icon.icns"
    ICON_OPTION="--icon=assets/app-icon.icns"
else
    echo "⚠️  No custom icon found (assets/app-icon.icns)"
    ICON_OPTION=""
fi
...
ADD_DATA_OPTIONS="$ADD_DATA_OPTIONS --add-data docs/project/warp.md:."
...
- Custom icon: $([ -f "assets/app-icon.icns" ] && echo "Yes (assets/app-icon.icns)" || echo "None")
...
- **Custom icon**: $([ -f "assets/app-icon.icns" ] && echo "Yes" || echo "None")
- **Build date**: $(date)
