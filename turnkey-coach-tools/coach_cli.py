...
import sys
import json
import glob # <-- Import the correct module
...
def add_note(client):
    ...
    try:
        os.chdir(client_dir)
        os.system(f"{editor} {note_filename}")
    finally:
        os.chdir(original_dir)

def clean_client_directory(client):
    """Interactively deletes temporary/generated files from a client's directory."""
    client_dir = os.path.join(CLIENT_DATA_DIR, str(client['id']))
    console.print(f"\nScanning [cyan]{client_dir}[/cyan] for temporary files...")

    files_to_delete = []
    # FIX: Removed "note_*.txt" from this list to protect your notes
    patterns_to_delete = ["plan_for_upload_*.json", "*_history_*.txt", "plan.txt"]

    for pattern in patterns_to_delete:
        full_pattern = os.path.join(client_dir, pattern)
        # FIX: Use glob.glob instead of tempfile.glob.glob
        files_to_delete.extend([os.path.basename(f) for f in glob.glob(full_pattern)])
    
    files_to_delete = sorted(list(set(files_to_delete)))

    if not files_to_delete:
        console.print("[green]No temporary files to delete.[/green]")
        console.input("Press Enter to continue.")
        return
